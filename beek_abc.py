...
for i in range(0,len(df)-2):
    if i == 0:
        for j in range (0, 5):
            if df[df.columns[j]].iloc[i] != df[df.columns[j]].iloc[i+1]:
                worksheet.write(i+2, df.columns.get_loc(df.columns[j])+1, df[df.columns[j]].iloc[i+1], format8)
...
for k in range(1,len(df)-2):
    if k%2 != 0:
        y = k+1
        for j in range (0, 5):
            if df[df.columns[j]].iloc[y] != df[df.columns[j]].iloc[y+1]:
                worksheet.write(y+2, df.columns.get_loc(df.columns[j])+1, df[df.columns[j]].iloc[y+1], format8)
    elif k%2 == 0:
        x = k+2
        for j in range (0, 5):
            if df[df.columns[j]].iloc[x] != df[df.columns[j]].iloc[x+1]:
                worksheet.write(x+2, df.columns.get_loc(df.columns[j])+1, df[df.columns[j]].iloc[x+1], format8)
