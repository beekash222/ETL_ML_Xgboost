...
########
  ...
  observeEvent(input$rpart4, {
    ...
    INDIVIDUAL_COUNT <- list()
    
    for(i in 1: nrow(file))
    {
      INDIVIDUAL_COUNT[i] <- paste("SELECT count(*) FROM",file$TABLE[i],"WHERE", file$SCENARIO.S[i])
    }
    
    INDIVIDUAL_COUNT <- unlist(INDIVIDUAL_COUNT)
    
    INTERSECT_COUNT <- list()
    
    for(i in 1: nrow(file))
    {
      if(i== 1)
      {
        INTERSECT_COUNT[i] <- paste("SELECT count(*) FROM",file$TABLE[i],"WHERE", file$SCENARIO.S[i])
      }
      else
      {
        INTERSECT_COUNT[i] <- paste("SELECT count(*) FROM",file$TABLE[i],"WHERE", file$SCENARIO.S[i],"INTERSECT",INTERSECT_COUNT[i-1]) 
      }
    }
    
    INTERSECT_COUNT <- unlist(INTERSECT_COUNT)
    
    final_query <- cbind(INDIVIDUAL_COUNT,INTERSECT_COUNT)
